...
async def get_messages_by_user_in_group(database: AsyncSession, user: User, group: Group) -> List[Message]:
    ...
    statemet = select(Message).where(Message.sender_id == user.user_id).where(Message.group_id == group.group_id)
...
async def make_message(database: AsyncSession, sender: User, group: Group,message: str) -> Message:
    """Make a new message"""
    message: Message = Message(message=message, sender_id=sender.user_id, group_id=group.group_id)
