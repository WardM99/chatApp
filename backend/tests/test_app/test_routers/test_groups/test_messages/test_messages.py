...
async def test_make_message(database_session: AsyncSession, auth_client: AuthClient):
    ...
    post_request = await auth_client.post(f"/groups/{group.group_id}/messages", json={"message": "Hi", "replyId": None})
    assert post_request.status_code == status.HTTP_201_CREATED
    data = post_request.json()
    assert data["messageId"]
    assert data["message"] == "Hi"
    assert data["replyId"] is None
    assert data["senderId"] == user.user_id
    assert data["groupId"] == group.group_id
...
async def test_make_reply(database_session: AsyncSession, auth_client: AuthClient):
    ...
    post_request = await auth_client.post(f"/groups/{group.group_id}/messages", json={"message": "How are you", "replyId": data_firest_message["messageId"]})
    assert post_request.status_code == status.HTTP_201_CREATED
    data = post_request.json()
    assert data["messageId"]
    assert data["message"] == "How are you"
    assert data["replyId"] == data_firest_message["messageId"]
    assert data["senderId"] == user.user_id
    assert data["groupId"] == group.group_id
...
async def test_make_message_not_logged_in(database_session: AsyncSession, test_client: AsyncClient):
    user: User = await make_user(database_session, "User1", "pw1")
    group: Group = await make_group(database_session, user, "Group1")
    post_request = await test_client.post(f"/groups/{group.group_id}/messages", json={"message": "Hi", "replyId": None})
    assert post_request.status_code == status.HTTP_401_UNAUTHORIZED
...
async def test_get_messages_in_group(database_session: AsyncSession, auth_client: AuthClient):
    ...
    post_request = await auth_client.post(f"/groups/{group.group_id}/messages", json={"message": "Hi", "replyId": None})
    assert post_request.status_code == status.HTTP_201_CREATED
    post_request = await auth_client.post(f"/groups/{group.group_id}/messages", json={"message": "How are yall", "replyId": None})
    assert post_request.status_code == status.HTTP_201_CREATED
...
async def test_change_message(database_session: AsyncSession, auth_client: AuthClient):
    ...
    put_request = await auth_client.put(f"/groups/{group.group_id}/messages/{message_id}", json={"message": "Hi!", "replyId": None})
