...
async def test_messages_by_user_pagination(database_session: AsyncSession):
    ...
    messages: List[Message] = await get_messages_by_user_in_group(database_session, user1, group1)
    assert len(messages) == PAGE_SIZE
    messages = await get_messages_by_user_in_group(database_session, user1, group1,page=2)
    assert len(messages) == (PAGE_SIZE-1)
    messages = await get_messages_by_user_in_group(database_session, user1, group1,page=3)
    assert len(messages) == 0
...
async def test_messages_pagination_by_user_wrong_page(database_session: AsyncSession):
    ...
    for i in range(PAGE_SIZE):
        await make_message(database_session, user1, group1, f"Message{i}")
    with pytest.raises(ValueError):
        await get_messages_by_user_in_group(database_session, user1, group1,page=0)
    with pytest.raises(ValueError):
        await get_messages_by_user_in_group(database_session, user1, group1, page=-1)